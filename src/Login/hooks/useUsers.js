...
import { loadingError ,addUser, loadingUsers, onCloseForm, onOpenForm, onUserSelectedForm, removeUser, updateUser, initialUserForm } from "../store/slices/users/usersSlice";
...
export const useUsers = () => {
    ...
    const handlerAddUser = async (user) => {
        ...
        try {
            ...
            Swal.fire(
                (user.id === 0) ? 'Usuario Creado!' : 'Usuario actualizado con exito',
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire(
                        'Usuario Eliminado!',
                        'El usuario ha sido elimiado con exito.',
                        'success'
                    )
                } catch (error) {
                    if (error.response?.status == 401) {
